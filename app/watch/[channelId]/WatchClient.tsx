...
export default function WatchClient({ channelId }: WatchClientProps) {
    ...
    // Fetch channels and auto-select from URL
    useEffect(() => {
        const fetchChannels = async () => {
            try {
                ...
                // Find and select channel from URL
                const channel = data.channels.find(c =>
                    c.id.toLowerCase().replace(/[^a-z0-9]/g, '-') === channelId.toLowerCase() ||
                    c.name.toLowerCase().replace(/[^a-z0-9]/g, '-') === channelId.toLowerCase()
                );

                if (channel) {
                    setSelectedChannel(channel);
                } else if (data.channels.length > 0) {
                    // Fallback to first channel if not found
    ...
    const handleChannelSelect = useCallback((channel: Channel) => {
        ...
        router.replace(`/watch/${slug}`, { scroll: false });
    }, [router]);
    ...
    return (
        <div className="app-container">
            ...
            {/* Main Content - Mobile First: Video on top */}
